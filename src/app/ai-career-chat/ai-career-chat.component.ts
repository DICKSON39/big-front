  ...
  ngOnInit(): void {
    this.authService.getUser().subscribe((user) => {
      if (user) {
        ...
        this.messages.push({
          from: 'ai',
          text: `💥 *BURP* Hey/Niaje, I’m “Dickson AI” — LMS-powered, interdimensional intelligence with a TypeScript hangover and questionable design choices.
...
Now come on , let’s pick a career path before I self-destruct again. We don’t have time for a resume crisis AND another evil AI uprising. Let’s gooo!`,
        });
      }
    });
  ...
  sendMessage() {
    ...
    if (
      this.awaitingProgressCourseSelection &&
      this.teacherCoursesForProgress.length > 0
    ) {
      const selectedIndex = parseInt(currentMessage);
      if (
        !isNaN(selectedIndex) &&
        selectedIndex > 0 &&
        selectedIndex <= this.teacherCoursesForProgress.length
      ) {
        const selectedCourse =
          this.teacherCoursesForProgress[selectedIndex - 1];
        const userId = this.authService.getUserId();

        this.awaitingProgressCourseSelection = false;

        this.courseService
          .getStudentsInCourseWithProgress(selectedCourse.id)
          .subscribe({
            next: (students: any[]) => {
              if (!students.length) {
                this.messages.push({
              ...
              const formattedProgress = students
                .map(
                  (s: any, i: number) =>
            ...
            error: (err) => {
              this.messages.push({
                from: 'ai',
                text: '❌ Failed to fetch progress. The server’s doing the moonwalk again.',
              });
              this.isLoading = false;
            },
          });
        ...
        this.messages.push({
          from: 'ai',
          text: '❓Please enter a valid course number from the list.',
        });
        this.isLoading = false;
        return;
      }
    ...
    this.authService
      ...
      .pipe(
        switchMap((user) => {
          ...
          // 👋 Greeting
          const greetingRegex =
            /\b(hi|hello|hey|yo|sup|greetings|what can you do)\b/i;
          if (greetingRegex.test(currentMessage)) {
            let roleGreeting = `👋 Oyaah ${this.userFullName}, what can I do for you today?`;
          ...
          // 📊 Role-based stats
          const statsRegex =
            /\b(my stats|dashboard|admin stats|teacher stats|show my insights?)\b/i;
          ...
          // 📈 Student progress
          const studentProgressRegex =
            /\b(progress|how are my students|student report|course progress)\b/i;
            ...
            return this.aiService.getDropdownCourses().pipe(
              switchMap((courses: any[]) => {
                if (!courses.length) {
                  this.messages.push({
                    from: 'ai',
                    text: `😬 You don't seem to have any courses yet.`,
                  });
                  this.isLoading = false;
                  return of(null);
                }
          ...
          // 📚 Teacher course list
          const teacherCourseRegex =
            /\b(my courses|my classes|what (did|have) i create(d)?|show my courses|my lessons|i created)\b/i;
          ...
          const courseListRegex =
            /(what|which)?\s*(courses|classes).*available|do you have|offer/i;
  ...
  submitQuizAnswers(quizMessage: Message) {
    if (
      !quizMessage.quizAnswers ||
      !this.allQuizAnswered(quizMessage.quizQuestions!, quizMessage.quizAnswers)
    ) {
  ...
  addAiPathResponse(res: AiSuggestionResponse) {
    const msg: Message =
      res.fallback && (res.suggested_courses?.length ?? 0) > 0
        ? {
            from: 'ai',
  ...
  loadOlderMessages() {
    ...
    this.authService
      ...
      .subscribe({
        next: (res) => {
          const mappedMessages: Message[] = (res.messages || []).map(
            (msg: any) => ({
              from: msg.from_role,
  ...
  getRoleBasedStats(): void {
    ...
    if (this.userRole === 1) {
      ...
      this.messages.push({
        from: 'ai',
        text: '🤖 Unknown role. I’m confused, Morty.',
      });
      this.isLoading = false;
      return;
    }
    ...
    statsCall$.subscribe({
      ...
      error: () => {
        this.messages.push({
          from: 'ai',
          text: '⚠️ Couldn’t fetch your stats. Interdimensional glitch?',
        });
        this.isLoading = false;
      },
    });
...
${
  Object.keys(data.revenue_by_method || {}).length
    ? Object.entries(data.revenue_by_method)
        .map(
...
${
  (data.revenue_by_course || []).length
    ? data.revenue_by_course
        .map(
          (c: any) => `• ${c.course_title}: $${(+c.total_earned).toFixed(2)}`,
        )
        .join('\n')
    : 'None yet! 😤'
}
