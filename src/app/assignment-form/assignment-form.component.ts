...
export class AssignmentFormComponent implements OnInit {
  ...
  constructor(
    ...
    public data: { preselectedCourseId: number | null },
  ) {}
      ...
      next: (res: any) => {
        ...
        if (this.data?.preselectedCourseId) {
          this.assignmentForm.patchValue({
            course_id: this.data.preselectedCourseId,
          });
        }
      },
      error: () => {
  ...
  onSubmit(e: Event): void {
    ...
    this.assignmentService.createAssignment(courseId, payload).subscribe({
      next: (res) => {
        this.snackBar.open(res.message || '✅ Assignment created!', 'Close', {
          duration: 3000,
        });
      ...
      error: (err) => {
        this.snackBar.open(
          err.error?.message || '❌ Failed to create assignment',
          'Close',
          { duration: 3000 },
        );
      },
