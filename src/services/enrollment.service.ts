...
export class EnrollmentService {
  private apiUrl =
    'https://school-online-backend.onrender.com/api/v1/enrollments'; // Update with your actual API URL
  ...
  enrollStudent(data: { user_id: number; course_id: number }) {
    return this.http.post(
      'https://school-online-backend.onrender.com/api/v1/enrollments/enroll',
      data,
      {
  ...
  getStudentsWithCourses(
    searchTerm: string = '',
  ): Observable<{ students: StudentWithCourses[] }> {
    let params = new HttpParams();
    if (searchTerm) {
      params = params.set('search', searchTerm);
    }
    return this.http.get<{ students: StudentWithCourses[] }>(
